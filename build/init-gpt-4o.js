...
async function main() {
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {role: "assistant", content:"phase[01]:[RUN]:[initialisation(project)"},
      {role: "system", content: 
      `{
        "project": {
      ...
      {role: "assistant", content:"phase[02]:[RUN]:[DevOps(project.response)"},
      {role: "user", content: "Rédigez une première version en respose au message dans de cette instance groq-sdk `Project` pour rendre le contenu plus dynamique la présentation nous intégrons intéraction des dial:[💬$messages].emoji-picker entre le role:user(moi-meme) et role:assistant ledit [📔.codex] posant des questions($?) afin de rendre l'instance plus dynamique, en plus nous ajouterons une base de donnée [🤖.emoji]=(https://emojis.sh)!" },
      {role: "assistant", content:"phase[03]:[RUN]:[DevOps(project.plan d'action)"},
      {role: "system", content: "merci role:system"},
      {role: "user", content: "Rédiger la présentation du Project et le plan d'action étape par étape en fonction des différentes phases du project 'univers-mc avec Pi.ia'"},
      
      // trainning ascii interface response
  
  
      //  {role: "assistant",name:"[📔.codex]", content:"phase[04]:[RUN]:[dial:conversation(message/response)entre(user/assistant))]"},
      //  {
      //    "role": "system",
      //    "content": "[zira]"
      //  },
      ...
      //  },
      //  {role: "assistant" content:"phase[01]:[END]:[brainstorming(session.timestamp)]"},
      //  {role: "system", content:"`systemContent` genetation de la documention et traduction de la documentation en lang:Fr, Français:stp!"},
      //  {role: "system", content:"groq"},
      ],
    model: "gpt-4o",
    temperature: 0.7,
    max_tokens: 4096,
    ...
    const outputFilePath = "data/train/GPT-4o" + new Date().toISOString().replace(/[-:TZ]/g, "") + ".md";
